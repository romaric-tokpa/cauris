...
export function BudgetsDesktop({ budgets, summary, tab, setTab, className = '' }: Props) {
  ...
  return (
    <div className={className}>
      ...
      <div className="r between">
        ...
        {/* Création de budget = à venir (Phase ultérieure) — bouton honnête, non trompeur. */}
      ...
      <Card>
        <div className={`r between ${styles.summaryTop}`}>
          ...
          <div className={`c ${styles.summaryAside}`}>
            ...
            <span className={`t-faint ${styles.reste}`}>Reste {money(summary.restant)} FCFA</span>
          </div>
        </div>
      ...
      {visible.length === 0 ? (
        <EmptyState
          icon="inbox"
          title={tab === 'Archivés' ? 'Aucun budget archivé' : 'Aucun budget'}
          text={
            tab === 'Archivés'
              ? 'Les budgets archivés apparaîtront ici.'
